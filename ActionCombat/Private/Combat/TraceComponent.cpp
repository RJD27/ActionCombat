...
void UTraceComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (const FTraceSockets Socket: Sockets)
	{
		...
		TArray<FHitResult> OutResults; // UE array comes packed with functions
		...
		if (bDebugMode)
		{
			...
			UKismetSystemLibrary::DrawDebugBox(
				...
				ShapeRotation.Rotator(), // needed because ShapeRotation is FQuat and not using degrees
				1.0f,
				2.0f
			);
		}
	...
	for (const FHitResult& Hit : AllResults) // & prevents making copies and just goes to the reference to save data
	{
